...
export default function PaymentPage() {
  ...
  const handleTimeExpired = useCallback(() => {
    setTimeExpired(true)
    paymentState.setError("PAYMENT_EXPIRED", "Payment time has expired. Please initiate a new payment.", false)
  }, [paymentState])
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white mb-2">Complete Your Payment</h1>
          <p className="text-slate-600 dark:text-slate-400">Select your preferred payment method to proceed</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="pt-6">
                ...
                {timeExpired && (
                  <Alert className="mb-6 bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800">
                    <AlertDescription className="text-red-800 dark:text-red-200 flex items-center justify-between">
                      <span>⏰ Payment window has expired. Please start a new payment.</span>
                      <Button
                ...
                {paymentState.status === "error" && paymentState.error && !timeExpired && (
                  <Alert className="mb-6 bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800">
                    <AlertDescription className="text-red-800 dark:text-red-200">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-semibold mb-1">✕ {paymentState.error.message}</p>
                          <p className="text-sm opacity-90">Error Code: {paymentState.error.code}</p>
                          {paymentState.retryCount > 0 && (
                            <p className="text-sm opacity-90 mt-1">Attempt {paymentState.retryCount} of 3</p>
                          )}
                        </div>
                ...
                >
                  <TabsList className="grid w-full grid-cols-3 mb-6">
                    <TabsTrigger value="SEPAY" disabled={timeExpired || paymentState.status === "success"}>
                      SEPAY
                    </TabsTrigger>
                    <TabsTrigger value="WEB3" disabled={timeExpired || paymentState.status === "success"}>
                      WEB3
                    </TabsTrigger>
                    <TabsTrigger value="PayFast" disabled={timeExpired || paymentState.status === "success"}>
                      PayFast
                    </TabsTrigger>
                  </TabsList>
          ...
          <div className="space-y-6">
            {/* Payment Summary */}
            <PaymentSummary orderData={orderData} paymentStatus={paymentState.status} />
